        ...
        for (n = 0; n < l.n; ++n)
        {
          ...
          else
          {
            if (best_iou > l.thresh)
            {
              ...
              if (l.classfix > 0)
              {
                delta_region_class(l.output, l.delta, index + 5, best_class_id,
                    l.classes, l.softmax_tree,
    ...
    float thresh, int* map, float tree_thresh, int relative, Detection* dets)
{
  ...
  for (i = 0; i < l.w * l.h; ++i)
  {
    ...
    for (n = 0; n < l.n; ++n)
    {
      ...
      int obj_index = EntryIndex(l, 0, n * l.w * l.h + i, l.coords);
      int box_index = EntryIndex(l, 0, n * l.w * l.h + i, 0);
      int mask_index = EntryIndex(l, 0, n * l.w * l.h + i, 4);
      ...
      int class_index =
          EntryIndex(l, 0, n * l.w * l.h + i, l.coords + !l.background);
      if (l.softmax_tree)
      {
        ...
        if (map)
        {
          ...
          {
            int class_index =
                EntryIndex(l, 0, n * l.w * l.h + i, l.coords + 1 + map[j]);
      ...
      else
      {
        if (dets[index].objectness)
        {
          ...
          {
            int class_index =
                EntryIndex(l, 0, n * l.w * l.h + i, l.coords + 1 + j);
            float prob = scale * predictions[class_index];
            dets[index].prob[j] = (prob > thresh) ? prob : 0;
          }
        }
