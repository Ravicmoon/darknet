...
void* fetch_in_thread(void* ptr)
{
  while (!custom_atomic_load_int(&flag_exit))
  {
    ...
    if (letter_box)
      in_s = get_image_from_stream_letterbox(
          cap, net.w, net.h, net.c, &in_img, dont_close_stream);
    else
      in_s = get_image_from_stream_resize(
          cap, net.w, net.h, net.c, &in_img, dont_close_stream);
    if (!in_s.data)
    {
...
void* detect_in_thread(void* ptr)
{
  while (!custom_atomic_load_int(&flag_exit))
  {
    ...
    if (letter_box)
      dets =
          get_network_boxes(&net, get_width_mat(in_img), get_height_mat(in_img),
              demo_thresh, demo_thresh, 0, 1, &nboxes, 1);  // letter box
    else
...
void demo(char* cfgfile, char* weightfile, float thresh, float hier_thresh,
    ...
    int frame_skip, char* prefix, char* out_filename, int dont_show,
    int ext_output, int letter_box_in, int time_limit_sec, int benchmark,
    int benchmark_layers)
{
  ...
  if (out_filename && !flag_exit)
  {
    ...
    output_video_writer = create_video_writer(out_filename, 'D', 'I', 'V', 'X',
        src_fps, get_width_mat(det_img), get_height_mat(det_img), 1);
    ...
    ++count;
    {
      ...
      if (!benchmark)
        custom_atomic_store_int(&run_fetch_in_thread,
            1);  // if (custom_create_thread(&fetch_thread, 0, fetch_in_thread,
                 // 0)) error("Thread creation failed");
      ...
      if (nms)
      {
        if (l.nms_kind == DEFAULT_NMS)
          ...
          diounms_sort(
              local_dets, local_nboxes, l.classes, nms, l.nms_kind, l.beta_nms);
      }
