...
void forward_yolo_layer(const layer l, network_state state)
{
  ...
  *(l.cost) = 0;
  ...
  // class_count, avg_obj / count, avg_anyobj / (l.w*l.h*l.n*l.batch), recall /
  // count, recall75 / count, count);
...
void forward_yolo_layer_gpu(const layer l, network_state state)
{
  ...
  for (b = 0; b < l.batch; ++b)
  {
    for (n = 0; n < l.n; ++n)
    {
      ...
      // if(y->1) x -> inf
      // if(y->0) x -> -inf
      activate_array_ongpu(l.output_gpu + index, 2 * l.w * l.h,
          LOGISTIC);  // x,y
      if (l.scale_x_y != 1)
...
{
  axpy_ongpu(
      l.batch * l.inputs, state.net.loss_scale, l.delta_gpu, 1, state.delta, 1);
}
#endif
