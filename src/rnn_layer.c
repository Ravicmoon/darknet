...
void forward_rnn_layer(layer l, NetworkState state)
{
  NetworkState s = {0};
  s.train = state.train;
...
void backward_rnn_layer(layer l, NetworkState state)
{
  NetworkState s = {0};
  s.train = state.train;
  s.workspace = state.workspace;
...
void forward_rnn_layer_gpu(layer l, NetworkState state)
{
  NetworkState s = {0};
  s.train = state.train;
  s.workspace = state.workspace;
...
void backward_rnn_layer_gpu(layer l, NetworkState state)
{
  NetworkState s = {0};
  s.train = state.train;
  s.workspace = state.workspace;
  ...
  for (i = l.steps - 1; i >= 0; --i)
  {
    ...
    copy_ongpu(l.hidden * l.batch, self_layer.delta_gpu, 1,
        input_layer.delta_gpu, 1);  // the same delta for Input and Self layers
