...
void forward_convolutional_layer_gpu(convolutional_layer l, NetworkState state)
{
      ...
      else
      {
        ...
        else*/
        {
          ...
          im2col_align_ongpu(state.input + i * l.c * l.h * l.w, l.c, l.h, l.w,
              l.size, l.stride, l.pad, l.align_workspace_gpu, l.bit_align);
          // cudaDeviceSynchronize();
        ...
        transpose_bin_gpu((unsigned char*)state.workspace,
            (unsigned char*)l.transposed_align_workspace_gpu, k, n, l.bit_align,
            new_ldb, 8);
        // cudaDeviceSynchronize();
      ...
      (l.c / l.groups) % 8 == 0 && l.n % 8 == 0 && l.groups <= 1 && l.size > 1)
  {
    ...
    CHECK_CUDNN(cudnnConvolutionForward(cudnn_handle(), &alpha,
        l.srcTensorDesc16, input16, l.weightDesc16, l.weights_gpu16, l.convDesc,
        l.fw_algo16, state.workspace, l.workspace_size, &beta,
  ...
  if (l.activation == SWISH)
    ...
    activate_array_normalize_channels_ongpu(l.output_gpu, l.outputs * l.batch,
        l.batch, l.out_c, l.out_w * l.out_h, l.output_gpu);
  else if (l.activation == NORM_CHAN_SOFTMAX)
    activate_array_normalize_channels_softmax_ongpu(l.output_gpu,
        l.outputs * l.batch, l.batch, l.out_c, l.out_w * l.out_h, l.output_gpu,
        0);
  else if (l.activation == NORM_CHAN_SOFTMAX_MAXVAL)
    activate_array_normalize_channels_softmax_ongpu(l.output_gpu,
        l.outputs * l.batch, l.batch, l.out_c, l.out_w * l.out_h, l.output_gpu,
...
void backward_convolutional_layer_gpu(convolutional_layer l, NetworkState state)
{
  if (l.antialiasing)
  {
    NetworkState s = {0};
  ...
  if (l.activation == SWISH)
    ...
    gradient_array_normalize_channels_softmax_ongpu(l.output_gpu,
        l.outputs * l.batch, l.batch, l.out_c, l.out_w * l.out_h, l.delta_gpu);
  else if (l.activation == NORM_CHAN)
      ...
      (l.c / l.groups) % 8 == 0 && l.n % 8 == 0 && l.groups <= 1 && l.size > 1)
  {
    ...
    if (state.delta)
    {
      ...
      // fp16)
      CHECK_CUDNN(
          cudnnConvolutionBackwardData(cudnn_handle(), &alpha, l.weightDesc16,
      ...
      if (l.xnor)
        gradient_array_ongpu(
            original_input, l.batch * l.c * l.h * l.w, HARDTAN, state.delta);
    }
  }
  else
  {
      ...
      if (state.delta)
      {
        ...
        if (l.xnor)
          gradient_array_ongpu(original_input + i * l.c * l.h * l.w,
              l.c * l.h * l.w, HARDTAN, state.delta + i * l.c * l.h * l.w);
      }
    }
  }
#endif
...
void assisted_excitation_forward_gpu(convolutional_layer l, NetworkState state)
{
  ...
  */
  ...
  if (l.adam)
  {
    ...
    adam_update_gpu(l.biases_gpu, l.bias_updates_gpu, l.bias_m_gpu,
        l.bias_v_gpu, l.B1, l.B2, l.eps, decay, learning_rate, l.n, batch, l.t);
    if (l.scales_gpu)
    {
      adam_update_gpu(l.scales_gpu, l.scale_updates_gpu, l.scale_m_gpu,
          l.scale_v_gpu, l.B1, l.B2, l.eps, decay, learning_rate, l.n, batch,
          l.t);
    }
  }
  else
  {
    ...
    axpy_ongpu(
        l.nweights, -decay * batch, l.weights_gpu, 1, l.weight_updates_gpu, 1);
    ...
    {
      axpy_ongpu(
          l.n, learning_rate / batch, l.scale_updates_gpu, 1, l.scales_gpu, 1);
  ...
  if (l.deform)
  {
    ...
    if (l.rotate)
      ...
      stretch_weights_gpu(
          l.weight_deform_gpu, l.weights_gpu, l.nweights, l.n, l.size, 0, 0);
    else if (l.stretch_sway)
