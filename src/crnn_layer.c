...
layer make_crnn_layer(int batch, int h, int w, int c, int hidden_filters,
    int output_filters, int groups, int steps, int size, int stride,
    int dilation, int pad, ACTIVATION activation, int batch_normalize, int xnor,
    int train)
{
  ...
  *(l.input_layer) = make_convolutional_layer(batch, steps, h, w, c,
      hidden_filters, groups, size, stride, stride, dilation, pad, activation,
  ...
  *(l.self_layer) = make_convolutional_layer(batch, steps, h, w, hidden_filters,
      hidden_filters, groups, size, stride, stride, dilation, pad, activation,
      batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  ...
  *(l.output_layer) = make_convolutional_layer(batch, steps, h, w,
      hidden_filters, output_filters, groups, size, stride, stride, dilation,
...
#ifdef GPU
  cuda_push_array(
      l.self_layer->output_gpu, l.self_layer->output, l.outputs * l.batch);
#endif  // GPU
}
    ...
    float momentum, float decay, float loss_scale)
{
  update_convolutional_layer_gpu(
      *(l.input_layer), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.self_layer), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.output_layer), batch, learning_rate, momentum, decay, loss_scale);
}
...
void backward_crnn_layer_gpu(layer l, NetworkState state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i)
  {
    ...
    {
      fix_nan_and_inf(
          output_layer.delta_gpu, output_layer.inputs * output_layer.batch);
      fix_nan_and_inf(
          self_layer.delta_gpu, self_layer.inputs * self_layer.batch);
      fix_nan_and_inf(
          input_layer.delta_gpu, input_layer.inputs * input_layer.batch);
    }
