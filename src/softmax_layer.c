...
void forward_softmax_layer(const softmax_layer l, NetworkState net)
{
  if (l.softmax_tree)
  {
    int i;
...
void backward_softmax_layer(const softmax_layer l, NetworkState net)
{
...
void forward_softmax_layer_gpu(const softmax_layer l, NetworkState net)
{
  if (l.softmax_tree)
  {
  ...
  {
    softmax_x_ent_gpu(
        l.batch * l.inputs, l.output_gpu, net.truth, l.delta_gpu, l.loss_gpu);
    if (l.softmax_tree)
    {
      mask_gpu_new_api(
...
void backward_softmax_layer_gpu(const softmax_layer layer, NetworkState net)
{
