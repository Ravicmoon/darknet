...
void ForwardUpsampleLayer(layer* l, NetworkState net)
{
...
void BackwardUpsampleLayer(layer* l, NetworkState state)
{
  ...
  {
    upsample_cpu(state.delta, l->w, l->h, l->c, l->batch, l->stride, 0,
        l->scale, l->delta);
  }
}
...
void ForwardUpsampleLayerGpu(layer* l, NetworkState state)
{
  ...
  if (l->reverse)
  {
    upsample_gpu(l->output_gpu, l->out_w, l->out_h, l->c, l->batch, l->stride,
        0, l->scale, state.input);
  }
  else
  {
    upsample_gpu(state.input, l->w, l->h, l->c, l->batch, l->stride, 1,
        l->scale, l->output_gpu);
  }
}
...
void BackwardUpsampleLayerGpu(layer* l, NetworkState state)
{
  ...
  {
    upsample_gpu(state.delta, l->w, l->h, l->c, l->batch, l->stride, 0,
        l->scale, l->delta_gpu);
  }
}
#endif
