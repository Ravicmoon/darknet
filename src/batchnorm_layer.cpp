...
void ForwardBatchnormLayerGpu(layer* l, NetworkState state)
{
  ...
  if (state.train)
  {
    ...
    if (l->batch_normalize == 2)
    {
      ...
      int const minibatch_index = state.net->current_subdivision + 1;
      int const max_minibatch_index = state.net->subdivisions;
      ...
      fast_v_cbn_gpu(l->output_gpu, l->mean_gpu, l->batch, l->out_c,
          ...
          l->rolling_mean_gpu, l->rolling_variance_gpu, inverse_variance,
          .00001);

      normalize_scale_bias_gpu(l->output_gpu, l->mean_gpu, l->variance_gpu,
          l->scales_gpu, l->biases_gpu, l->batch, l->out_c, l->out_h * l->out_w,
          inverse_variance, .00001f);
