...
layer make_conv_lstm_layer(int batch, int h, int w, int c, int output_filters,
    int groups, int steps, int size, int stride, int dilation, int pad,
    ACTIVATION activation, int batch_normalize, int peephole, int xnor,
    int train)
{
  ...
  *(l.uf) = make_convolutional_layer(batch, steps, h, w, c, output_filters,
      groups, size, stride, stride, dilation, pad, activation, batch_normalize,
      0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.uf->batch = batch;
  ...
  *(l.wf) = make_convolutional_layer(batch, steps, h, w, output_filters,
      output_filters, groups, size, stride, stride, dilation, pad, activation,
      batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.wf->batch = batch;
  if (l.workspace_size < l.wf->workspace_size)
    l.workspace_size = l.wf->workspace_size;

  l.wi = (layer*)xcalloc(1, sizeof(layer));
  *(l.wi) = make_convolutional_layer(batch, steps, h, w, output_filters,
      output_filters, groups, size, stride, stride, dilation, pad, activation,
      batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.wi->batch = batch;
  ...
  *(l.wg) = make_convolutional_layer(batch, steps, h, w, output_filters,
      output_filters, groups, size, stride, stride, dilation, pad, activation,
      batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.wg->batch = batch;
  ...
  *(l.wo) = make_convolutional_layer(batch, steps, h, w, output_filters,
      output_filters, groups, size, stride, stride, dilation, pad, activation,
      batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.wo->batch = batch;
  ...
  if (l.peephole)
  {
    *(l.vf) = make_convolutional_layer(batch, steps, h, w, output_filters,
        output_filters, groups, size, stride, stride, dilation, pad, activation,
        batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
    l.vf->batch = batch;
  ...
  if (l.peephole)
  {
    *(l.vi) = make_convolutional_layer(batch, steps, h, w, output_filters,
        output_filters, groups, size, stride, stride, dilation, pad, activation,
        batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
    l.vi->batch = batch;
  ...
  if (l.peephole)
  {
    *(l.vo) = make_convolutional_layer(batch, steps, h, w, output_filters,
        output_filters, groups, size, stride, stride, dilation, pad, activation,
        batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
    l.vo->batch = batch;
...
void forward_conv_lstm_layer(layer l, NetworkState state)
{
  NetworkState s = {0};
  s.train = state.train;
...
void backward_conv_lstm_layer(layer l, NetworkState state)
{
  NetworkState s = {0};
  s.train = state.train;
  s.workspace = state.workspace;
    ...
    float momentum, float decay, float loss_scale)
{
  if (l.peephole)
  {
    update_convolutional_layer_gpu(
        *(l.vf), batch, learning_rate, momentum, decay, loss_scale);
    update_convolutional_layer_gpu(
        *(l.vi), batch, learning_rate, momentum, decay, loss_scale);
    update_convolutional_layer_gpu(
        *(l.vo), batch, learning_rate, momentum, decay, loss_scale);
  }
  update_convolutional_layer_gpu(
      *(l.wf), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.wi), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.wg), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.wo), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.uf), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.ui), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.ug), batch, learning_rate, momentum, decay, loss_scale);
  update_convolutional_layer_gpu(
      *(l.uo), batch, learning_rate, momentum, decay, loss_scale);
}

void forward_conv_lstm_layer_gpu(layer l, NetworkState state)
{
  NetworkState s = {0};
  s.train = state.train;
  s.workspace = state.workspace;
  ...
  for (i = 0; i < l.steps; ++i)
  {
    ...
    // f = wf + uf + vf
    add_3_arrays_activate(wf.output_gpu, uf.output_gpu,
        (l.peephole) ? vf.output_gpu : NULL, l.outputs * l.batch, LOGISTIC,
        l.f_gpu);
    ...
    // i = wi + ui + vi
    add_3_arrays_activate(wi.output_gpu, ui.output_gpu,
        (l.peephole) ? vi.output_gpu : NULL, l.outputs * l.batch, LOGISTIC,
        l.i_gpu);
    ...
    add_3_arrays_activate(
        wg.output_gpu, ug.output_gpu, NULL, l.outputs * l.batch, TANH, l.g_gpu);
    ...
    add_3_arrays_activate(wo.output_gpu, uo.output_gpu,
        (l.peephole) ? vo.output_gpu : NULL, l.outputs * l.batch, LOGISTIC,
        l.o_gpu);
    ...
    simple_copy_ongpu(l.outputs * l.batch, l.h_gpu,
...
void backward_conv_lstm_layer_gpu(layer l, NetworkState state)
{
  ...
  // fill_ongpu(l.outputs * l.batch, 0, l.dc_gpu, 1);   //  dont use
  const int sequence = GetSequenceValue(state.net);
  ...
  for (i = l.steps - 1; i >= 0; --i)
  {
    ...
    // use
      ...
      simple_copy_ongpu(
          l.outputs * l.batch, l.last_prev_cell_gpu, l.prev_cell_gpu);
    ...
    // use
      ...
      simple_copy_ongpu(
          l.outputs * l.batch, l.last_prev_state_gpu, l.prev_state_gpu);
    ...
    simple_copy_ongpu(
        l.outputs * l.batch, l.delta_gpu, l.temp3_gpu);  // temp3 = delta

    simple_copy_ongpu(l.outputs * l.batch, l.c_gpu, l.temp_gpu);
    activate_array_ongpu(
        l.temp_gpu, l.outputs * l.batch, TANH);  // temp  = tanh(c)

    simple_copy_ongpu(l.outputs * l.batch, l.temp3_gpu, l.temp2_gpu);
    mul_ongpu(
        l.outputs * l.batch, l.o_gpu, 1, l.temp2_gpu, 1);  // temp2 = delta * o

    gradient_array_ongpu(l.temp_gpu, l.outputs * l.batch, TANH,
