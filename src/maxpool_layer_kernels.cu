...
extern "C" void forward_maxpool_layer_gpu(
    maxpool_layer layer, NetworkState state)
{
  if (layer.maxpool_depth)
  {
    ...
    forward_maxpool_depth_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
        get_cuda_stream()>>>(n, layer.w, layer.h, layer.c, layer.out_c,
        layer.batch, state.input, layer.output_gpu, layer.indexes_gpu);
  ...
  if (!state.train && layer.stride == layer.size)
  {
    ...
    maxpool_status = cudnnPoolingForward(cudnn_handle(), layer.poolingDesc,
        &alpha, layer.srcTensorDesc, state.input, &beta, layer.dstTensorDesc,
        layer.output_gpu);
...
#endif
  {
    ...
    forward_maxpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
        get_cuda_stream()>>>(n, layer.h, layer.w, layer.c, layer.stride_x,
        layer.stride_y, layer.size, layer.pad, state.input, layer.output_gpu,
        layer.indexes_gpu);
    CHECK_CUDA(cudaPeekAtLastError());
  }
    ...
    maxpool_layer layer, NetworkState state)
{
  ...
  if (layer.maxpool_depth)
  {
    ...
    backward_maxpool_depth_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
        get_cuda_stream()>>>(n, layer.w, layer.h, layer.c, layer.batch,
  ...
  backward_maxpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
      get_cuda_stream()>>>(n, layer.h, layer.w, layer.c, layer.stride_x,
      layer.stride_y, layer.size, layer.pad, layer.delta_gpu, state.delta,
      layer.indexes_gpu);
  CHECK_CUDA(cudaPeekAtLastError());
}
...
extern "C" void backward_local_avgpool_layer_gpu(
    maxpool_layer layer, NetworkState state)
{
  size_t n = layer.h * layer.w * layer.c * layer.batch;
