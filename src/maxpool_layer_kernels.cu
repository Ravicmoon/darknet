...
void ForwardLocalAvgpoolLayerGpu(layer* l, NetworkState state)
{
#ifdef CUDNN_DISABLED
  if (!state.train && l->stride == l->size)
  {
...
#endif
  {
    ...
    forward_local_avgpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
        get_cuda_stream()>>>(n, l->h, l->w, l->c, l->stride_x, l->stride_y,
        l->size, l->pad, state.input, l->output_gpu);
    CHECK_CUDA(cudaPeekAtLastError());
  }
}

void BackwardLocalAvgpoolLayerGpu(layer* l, NetworkState state)
{
  size_t n = l->h * l->w * l->c * l->batch;

  backward_local_avgpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0,
      get_cuda_stream()>>>(n, l->h, l->w, l->c, l->stride_x, l->stride_y,
      l->size, l->pad, l->delta_gpu, state.delta);
  CHECK_CUDA(cudaPeekAtLastError());
}
