...
void Hungarian::HungarianAlgo(BipartiteGraph& _bg, vector<VID>& _S, vector<VID>& _T, vector<VID>& _N, vector<EID>& _EG, vector<EID>& _M)
{
	if(m_bVerbose)
	{
			...
			if(!NeedReLabel(_T, _N))
			{
				...
				if(m_bVerbose)
				{
					...
					if(!bg.GetTask(y)->GetMatched())
						cout<<"Task VID "<<y<<" has NOT been matched, then "<<u<<"-->"<<y<<" is an augmenting path, searching..."<<endl;
					else
...
void Hungarian::HungarianAlgo(void)
{
	HungarianAlgo(bg, S, T, N, EG, M);
}

void Hungarian::InitNewRoot(BipartiteGraph& _bg, VID root, vector<VID>& _S, vector<VID>& _T)
{ 
...
void Hungarian::RefreshBG(BipartiteGraph& _bg, const vector<VID>& _S, const vector<VID>& _T, vector<VID>& _N, vector<EID>& _EG, vector<EID>& _M)
{
...
void Hungarian::ExtendTree(BipartiteGraph& _bg, VID x, vector<VID>& _S, vector<VID>& _T)
{
...
vector<EID> Hungarian::BFSAugmentingPath(BipartiteGraph& _bg, VID x, VID y)
{
	...
	while(dq.size() && !found)
	{
		...
		else if(v.GetObj() == "AGENT")
		{
			...
			{
				if(!_bg.GetMatrix(v.GetVID(), task_index)->GetMatchedFlag() && _bg.GetMatrix(v.GetVID(), task_index)->GetAdmissibleFlag() && !_bg.GetTask(task_index)->path.size())
				{
					if(m_bVerbose)
					{
						if(task_index == y)
...
vector<VID> Hungarian::FindNeighbors(const vector<EID>& _EG, const vector<VID>& _S)
{
	vector<VID> _N;
	_N.clear();
