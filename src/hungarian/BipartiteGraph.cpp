...
bool BipartiteGraph::CheckFeasibility(Matrix& _m, vector<Vertex>& _agents, vector<Vertex>& _tasks)
{
	//check the size for args
	if(_m.size() != _agents.size() || _m[0].size() != _tasks.size())
	{
		throw exception("[BipartiteGraph::CheckFeasibility] size discrepency found during feasibility checking");
	}
	...
	for(size_t i=0; i<_m.size(); i++)
	{
		for(size_t j=0; j<_m[0].size(); j++)
		{
			if(_m[i][j].GetWeight() - (_agents[i].GetLabel() + _tasks[j].GetLabel()) > DOUBLE_EPSILON )
			{
				...
				{
					throw exception("[BipartiteGraph::CheckFeasibility] abels are not feasible any more!");
...
bool BipartiteGraph::CheckFeasibility(void)
{
	...
	if (!res)
		throw exception("[BipartiteGraph::CheckFeasibility] bipartite graph is not feasible!");
