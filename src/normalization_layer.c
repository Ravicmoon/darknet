    ...
    float alpha, float beta, float kappa)
{
  ...
  l.forward = ForwardNormalizationLayer;
  l.backward = BackwardNormalizationLayer;
#ifdef GPU
  l.forward_gpu = ForwardNormalizationLayerGpu;
  l.backward_gpu = BackwardNormalizationLayerGpu;
...
void ForwardNormalizationLayer(layer* l, NetworkState state)
{
  ...
  for (b = 0; b < l->batch; ++b)
  {
    ...
    for (k = 1; k < l->c; ++k)
    {
      ...
      if (prev >= 0)
        axpy_cpu(
            w * h, -l->alpha, squared + w * h * prev, 1, norms + w * h * k, 1);
      if (next < l->c)
        axpy_cpu(
            w * h, l->alpha, squared + w * h * next, 1, norms + w * h * k, 1);
    }
  }
