...
void transpose_32x32_bits_reversed_diagonale(
...
void transpose8rS32_reversed_diagonale(
...
static inline float _castu32_f32(uint32_t a) { return *((float*)&a); }
...
void gemm_nn_bin_32bit_packed(int M, int N, int K, float ALPHA, uint32_t* A,
    int lda, uint32_t* B, int ldb, float* C, int ldc, float* mean_arr)
{
  int i;
#pragma omp parallel for
  for (i = 0; i < M; ++i)
  {  // l.n
    ...
    for (s = 0; s < K; ++s)  // l.size*l.size*l.c/32  or (l.size*l.size*l.c)
    {
      ...
      for (j = 0; j < N - 8; j += 8)
      {
        ...
        __m256 count =
            _mm256_setr_ps(popcnt_32(_mm256_extract_epi32(xnor256, 0)),
    ...
    float* weights, float* input, float* output, float* mean)
{
  ...
  {
    *((__m256*)&weights[i]) = _mm256_and_ps(
        *((__m256*)&weights[i]), _mm256_castsi256_ps(all256_sing1));
  }
  ...
  __m256i all256_last_zero = _mm256_set_epi32(0xFFFFFFFF, 0xFFFFFFFF,
...
static inline int popcnt128(__m128i n)
{
  ...
  return _mm_popcnt_u64(_mm_cvtsi128_si64(n)) +
         _mm_popcnt_u64(_mm_cvtsi128_si64(n_hi));
#else
...
{
  __m256i lookup = _mm256_setr_epi8(0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3,
...
static inline void xnor_avx2_popcnt(
    __m256i a_bit256, __m256i b_bit256, __m256i* count_sum)
{
  ...
  {  // l.n - filters [16 - 55 - 1024]
    ...
    for (i_d = 0; i_d < 2; ++i_d)
    {
      ...
      {  // out_h*out_w - one channel output size [169 - 173056]
        ...
        {  // l.size*l.size*l.c - one filter size [27 - 9216]
          __m256i a_bit256_0 =
              _mm256_loadu_si256((__m256i*)(A + ((i + i_d + 0) * lda + k) / 8));
          __m256i b_bit256_0 =
              _mm256_loadu_si256((__m256i*)(B + ((j + 0) * ldb + k) / 8));
...
void im2col_cpu_custom_transpose(float* data_im, int channels, int height,
    int width, int ksize, int stride, int pad, float* data_col, int ldb_align)
{
...
void im2col_cpu_custom_align(float* data_im, int channels, int height,
    int width, int ksize, int stride, int pad, float* data_col, int bit_align)
{
  int c;
  const int height_col = (height + 2 * pad - ksize) / stride + 1;
  const int width_col = (width + 2 * pad - ksize) / stride + 1;
...
static inline void transpose4x4_SSE(
    float* A, float* B, const int lda, const int ldb)
{
...
void forward_maxpool_layer_avx(float* src, float* dst, int* indexes, int size,
    int w, int h, int out_w, int out_h, int c, int pad, int stride, int batch)
{
...
static inline void transpose_scalar_block(
    float* A, float* B, const int lda, const int ldb, const int block_size)
{
  int i;
  //#pragma omp parallel for
...
void gemm_nn_bin_transposed_32bit_packed(int M, int N, int K, float ALPHA,
    uint32_t* A, int lda, uint32_t* B, int ldb, float* C, int ldc,
    float* mean_arr)
{
  int i;
#pragma omp parallel for
...
void convolution_repacked(uint32_t* packed_input, uint32_t* packed_weights,
    float* output, int w, int h, int c, int n, int size, int pad, int new_lda,
    float* mean_arr)
{
  int fil;
// filter index
        ...
        for (x = 0; x < w; ++x)
        {
          ...
          for (f_y = 0; f_y < size; ++f_y)
          {
            ...
            for (f_x = 0; f_x < size; ++f_x)
            {
              ...
              uint32_t input = ((
                  uint32_t*)packed_input)[chan * w * h + input_y * w + input_x];
    ...
    int lda, float* B_gpu, int ldb, float BETA, float* C_gpu, int ldc)
{
  ...
  cudaError_t status = (cudaError_t)cublasSgemm(handle,
      (TB ? CUBLAS_OP_T : CUBLAS_OP_N), (TA ? CUBLAS_OP_T : CUBLAS_OP_N), N, M,
