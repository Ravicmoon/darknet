    ...
    layer* l, int batch, float learning_rate, float momentum, float decay)
{
  ...
  {
    axpy_cpu(
        l->outputs, learning_rate / batch, l->scale_updates, 1, l->scales, 1);
...
void BackwardConnectedLayer(layer* l, NetworkState state)
{
  ...
  if (l->batch_normalize)
  {
    ...
    mean_delta_cpu(
        l->delta, l->variance, l->batch, l->outputs, 1, l->mean_delta);
    variance_delta_cpu(l->x, l->delta, l->mean, l->variance, l->batch,
        l->outputs, 1, l->variance_delta);
...
void ForwardConnectedLayerGpu(layer* l, NetworkState state)
{
  ...
  CHECK_CUDNN(cudnnConvolutionForward(cudnn_handle(), &alpha, l->srcTensorDesc,
      ...
      state.workspace, l->workspace_size, &beta, l->dstTensorDesc,
      l->output_gpu));
#else   // CUDNN
  int m = l->batch;
  int k = l->inputs;
...
void UpdateConnectedLayerGpu(layer* l, int batch, float learning_rate_init,
    float momentum, float decay, float loss_scale)
{
  ...
  {
    scal_ongpu(
        l->inputs * l->outputs, 1.0 / loss_scale, l->weight_updates_gpu, 1);
...
void PushConnectedLayer(layer* l)
{
  ...
  cuda_push_array(
      l->weight_updates_gpu, l->weight_updates, l->inputs * l->outputs);
