    ...
    int stride, int pad, float* data_col)
{
  ...
  im2col_gpu_kernel<<<(num_kernels + BLOCK - 1) / BLOCK, BLOCK, 0,
      get_cuda_stream()>>>(num_kernels, im, height, width, ksize, pad, stride,
  ...
  for (; index < n; index += blockDim.x * gridDim.x)
  {
    ...
    for (int i = 0; i < ksize; ++i)
    {
      for (int j = 0; j < ksize; ++j)
      {
        ...
        // == 0) *((unsigned int*)data_col_ptr_32) = bit_mask; use atomicOr() //
        // *dst_ptr |= (mask << (col_index % 8)); data_col_ptr_32 += bit_align /
        // 32;
    ...
    int ksize, int stride, int pad, float* data_col, int bit_align)
{
  ...
  im2col_align_gpu_kernel<<<(num_kernels + BLOCK - 1) / BLOCK, BLOCK, 0,
      get_cuda_stream()>>>(num_kernels, im, height, width, ksize, pad, stride,
      height_col, width_col, data_col, bit_align);
  ...
  // for (; index < n; index += blockDim.x*gridDim.x)
  {
    ...
    // for (int h_out = 0; h_out < height_col; ++h_out)
    {
      ...
      // for (int w_out = 0; w_out < (width_col); w_out += 32)
      {
        ...
        if (i >= ksize)
          send_wh_index = height_col * width_col;
...
__global__ void float_to_bit_gpu_kernel(
    float* src, unsigned char* dst, size_t size)
{
  ...
  for (i = 0; i < 32; ++i)
  {
    if ((index + i * 1024) < size)
      src_val = src[index + i * 1024];
    else
      src_val = 0;
...
void float_to_bit_gpu(float* src, unsigned char* dst, size_t size)
{
  ...
  float_to_bit_gpu_kernel<<<num_blocks, 1024, 0, get_cuda_stream()>>>(
      src, dst, size);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
__device__ void transpose8rS32_reversed_diagonale(
    unsigned char* A, unsigned char* B, int m, int n)
{
  unsigned x, y, t;
...
__global__ void transpose_bin_gpu_kernel(unsigned char* A, unsigned char* B,
    const int n, const int m, const int lda, const int ldb,
    ...
    // for (j = 0; j < m - 8; j += 8)
    {
      ...
      if (j < m)
      {
        ...
        transpose8rS32_reversed_diagonale(
            &A[a_index / 8], &B[b_index / 8], lda / 8, ldb / 8);
      }
      // else if (j < m) {
      //    for (; j < m; ++j) {
...
extern "C" {
__device__ void transpose_32x32_bits_reversed_diagonale(
    uint32_t* A, uint32_t* B, int m, int n)
{
  // unsigned A_tmp[32];
  // int i;
  //#pragma unroll
...
void transpose_bin_gpu(unsigned char* A, unsigned char* B, const int n,
    const int m, const int lda, const int ldb, const int block_size)
{
  // int size = n*m/ (8*8) + 1;
  ...
  // transpose_bin_gpu_kernel << <num_blocks, BLOCK, 0, get_cuda_stream() >>
  // >(A, B, n, m, lda, ldb, block_size);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
__global__ void transpose_uint32_kernel_2(uint32_t* src, uint32_t* dst,
    int src_h, int src_w, int src_align, int dst_align)
{
...
__global__ void repack_input_kernel(
    float* input, float* re_packed_input, int w, int h, int c)
{
  int index = blockIdx.x * blockDim.x + threadIdx.x;
...
// 32 channels -> 1 channel (with 32 floats)
// 256 channels -> 8 channels (with 32 floats)
__global__ void repack_input_kernel_2(
    float* input, float* re_packed_input, int w, int h, int c)
{
...
// 32 channels -> 1 channel (with 32 floats)
// 256 channels -> 8 channels (with 32 floats)
__global__ void repack_input_kernel_bin(
    float* input, uint32_t* re_packed_input_bin, int w, int h, int c)
{
...
__global__ void fill_int8_gpu_kernel(
    unsigned char* src, unsigned char val, size_t size)
{
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index < size)
    src[index] = 0;
...
{
  const int num_blocks = size / BLOCK + 1;
  fill_int8_gpu_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(
      src, val, size);
  CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
__device__ __host__ static inline ulonglong4 xor_int256(
    ulonglong4 a, ulonglong4 b)
{
  ulonglong4 res;
  res.w = (a.w ^ b.w);
  res.x = (a.x ^ b.x);
...
__global__ void gemm_nn_custom_bin_mean_transposed_tensor_kernel(int M, int N,
    int K, unsigned char* A, int lda, unsigned char* B, int ldb, float* C,
    int ldc, float* mean_arr, float* bias_arr, int leaky_activation,
    ...
    if (i < M)  // if (i < M)  // l.n - filters [16 - 55 - 1024]
    {
           ...
           k += 128)  // l.size*l.size*l.c - one filter size [27 - 144 - 9216]
      {
        ...
        wmma::load_matrix_sync(
            a_frag, (uint32_t*)(A + A_cur_index), lda);  // lda = M

        wmma::load_matrix_sync(
            b_frag, (uint32_t*)(B + B1_cur_index), ldb);    // ldb = K
        wmma::bmma_sync(c1_frag, a_frag, b_frag, c1_frag);  // XOR-GEMM

        wmma::load_matrix_sync(
            b_frag, (uint32_t*)(B + B2_cur_index), ldb);    // ldb = K
...
__global__ void gemm_nn_custom_bin_mean_transposed_gpu_kernel(int M, int N,
    int K, unsigned char* A, int lda, unsigned char* B, int ldb, float* C,
    int ldc, float* mean_arr, float* bias_arr, int leaky_activation,
    float* shortcut_in_gpu, float* shortcut_out_gpu)
{
...
void gemm_nn_custom_bin_mean_transposed_gpu(int M, int N, int K,
    unsigned char* A, int lda, unsigned char* B, int ldb, float* C, int ldc,
    float* mean_arr, float* bias, int leaky_activation, float* shortcut_in_gpu,
  ...
  */
  ...
  if (1)
  {
    ...
    gemm_nn_custom_bin_mean_transposed_tensor_kernel<<<num_blocks, BLOCK, 0,
        get_cuda_stream()>>>(M, N, K, A, lda, B, ldb, C, ldc, mean_arr, bias,
        leaky_activation, shortcut_in_gpu, shortcut_out_gpu);
...
__global__ void convolve_gpu_kernel(float* input, float* weights, float* output,
    int in_w, int in_h, int in_c, int n, int size, int pad)
{
  int index = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void convolve_bin_gpu_kernel(float* input, float* weights,
    float* output, int in_w, int in_h, int in_c, int n, int size, int pad,
    ...
    float* mean_arr_gpu)
{
  ...
  convolve_bin_gpu_kernel<<<num_blocks, BLOCK, 0, get_cuda_stream()>>>(input,
      weights, output, in_w, in_h, in_c, n, size, pad, new_lda, mean_arr_gpu);
  CHECK_CUDA(cudaPeekAtLastError());
}
    ...
    const int dilation_h, const int dilation_w, float* data_col)
{
  ...
  im2col_gpu_kernel_ext<<<CAFFE_GET_BLOCKS(num_kernels),
      CAFFE_CUDA_NUM_THREADS>>>(num_kernels, data_im, height, width, kernel_h,
