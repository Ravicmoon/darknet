...
{
#ifdef GPU
  if (gpu_index >= 0) return GetNetworkOutputGpu(net);
#endif
  int i;
  for (i = net->n - 1; i > 0; --i)
  {
...
{
  float* out = GetNetworkOutput(&net);
  int k = GetNetworkOutputSize(&net);
...
{
#ifdef GPU
  if (gpu_index >= 0) return TrainNetworkDatumGpu(net, x, y);
#endif
...
{
  int size = GetNetworkOutputSize(&net);
  float* out = GetNetworkOutput(&net);
...
{
#ifdef GPU
  if (gpu_index >= 0) return NetworkPredictGpu(net, input);
#endif
...
Detection* make_network_boxes_batch(
    Network* net, float thresh, int* num, int batch)
{
  int i;
  layer l = net->layers[net->n - 1];
...
void fill_network_boxes_batch(Network* net, int w, int h, float thresh,
    float hier, int* map, int relative, Detection* dets, int letter, int batch)
{
  int prev_classes = -1;
  int j;
  for (j = 0; j < net->n; ++j)
  {
    ...
    if (l.type == YOLO)
    {
      int count = get_yolo_detections_batch(
          l, w, h, net->w, net->h, thresh, map, relative, dets, letter, batch);
      dets += count;
      if (prev_classes < 0)
        prev_classes = l.classes;
      else if (prev_classes != l.classes)
...
matrix network_predict_data_multi(Network net, data test, int n)
{
  ...
  for (i = 0; i < test.X.rows; i += net.batch)
  {
    ...
    for (m = 0; m < n; ++m)
    {
      float* out = NetworkPredict(&net, X);
      for (b = 0; b < net.batch; ++b)
      {
        if (i + b == test.X.rows) break;
        for (j = 0; j < k; ++j)
