...
connected_layer make_connected_layer(int batch, int steps, int inputs,
    int outputs, ACTIVATION activation, int batch_normalize)
{
...
void forward_connected_layer(connected_layer l, NetworkState state)
{
  int i;
...
void backward_connected_layer(connected_layer l, NetworkState state)
{
  int i;
...
void forward_connected_layer_gpu(connected_layer l, NetworkState state)
{
...
void backward_connected_layer_gpu(connected_layer l, NetworkState state)
{
  int i;
  ...
  CHECK_CUDNN(cudnnConvolutionBackwardFilter(cudnn_handle(), &one,
      l.srcTensorDesc, state.input, l.ddstTensorDesc, l.delta_gpu, l.convDesc,
      l.bf_algo, state.workspace, l.workspace_size, &one, l.dweightDesc,
  ...
  if (state.delta)
  {
    ...
    CHECK_CUDNN(cudnnConvolutionBackwardData(cudnn_handle(), &one, l.weightDesc,
        l.weights_gpu, l.ddstTensorDesc, l.delta_gpu, l.convDesc, l.bd_algo,
        state.workspace, l.workspace_size, &one, l.dsrcTensorDesc,
        state.delta));
  }
#else   // CUDNN
