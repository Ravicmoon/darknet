...
void delta_region_class(float* output, float* delta, int index, int class_id,
    int classes, tree* hier, float scale, float* avg_cat, int focal_loss)
{
  int i, n;
  if (hier)
  {
  ...
  *(l->cost) = 0;
  for (b = 0; b < l->batch; ++b)
  {
    if (l->softmax_tree)
    {
      ...
      {
        Box truth(state.truth + t * 5 + b * l->truths);
        if (!truth.x)
          break;  // continue;
        ...
        for (n = 0; n < l->n; ++n)
        {
          ...
          else
          {
            if (best_iou > l->thresh)
            {
              ...
              if (l->classfix > 0)
              {
                delta_region_class(l->output, l->delta, index + 5,
                    best_class_id, l->classes, l->softmax_tree,
                    l->class_scale *
                        (l->classfix == 2 ? l->output[index + 4] : 1),
    ...
    for (t = 0; t < l->max_boxes; ++t)
    {
      ...
      delta_region_class(l->output, l->delta, best_index + 5, class_id,
          l->classes, l->softmax_tree, l->class_scale, &avg_cat, l->focal_loss);
      ++count;
      ++class_count;
    }
  }
...
void ForwardRegionLayerGpu(layer* l, NetworkState state)
{
  ...
  ForwardRegionLayer(l, cpu_state);
